...
WeatherService::WeatherService()
  : _notifier(BOT_TOKEN, TELEGRAM_CHAT_ID) {
}
...
void WeatherService::updateForecast(const char* cityName) {
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (!error) {
      ...
      Serial.println(F("[Weather] Today Code"));
      Serial.println(weatherCode);
      _notifier.sendWeatherAlert(weatherCode);
    }
  }
  else{
    Serial.println(F("[Weather] Could not send req"));
  }
